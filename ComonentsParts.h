...
struct LetterSubComponent
{
    ...
    void read(char str[MAX_ROW_SIZE], int rowIndex)
    {
        ...
        while(circleValue)
        {
            ...
            else if(ColtStangaSus.x==-1)
            {
                ColtStangaSus.x=atoi(circleValue);
            }
            else if(ColtStangaSus.y==-1)
            {
                ColtStangaSus.y=atoi(circleValue);
            }
            else
            {
    ...
    void draw(Point origin,int rotatedAt, float scaledAt, sf::RenderWindow &win)
    {
        ...
        letterComponent.setPosition(ColtStangaSus.x+origin.x,ColtStangaSus.y+origin.y);
...
struct CircleSubComponent
{
    ...
    void draw(Point origin,int rotatedAt, float scaledAt, sf::RenderWindow &win)
    {
        ...
        circle.setPosition(ColtStangaSus.x+origin.x,ColtStangaSus.y+origin.y);
...
struct RectangleSubComponent
{
    ...
    void read(char str[MAX_ROW_SIZE], int rowIndex)
    {
        ...
        while(rectangleValue)
        {
            ...
            {
                ColtStangaSus.x=atoi(rectangleValue);
            }
            else if(ColtStangaSus.y==-1)
            {
                ColtStangaSus.y=atoi(rectangleValue);
            }
            else
            {
    ...
    void draw(Point origin,int rotatedAt,float scaledAt, sf::RenderWindow &win)
    {
        ...
        rectangle.setPosition(ColtStangaSus.x+origin.x,ColtStangaSus.y+origin.y);
...
struct ConnectionSubComponent
{
    ...
    int OutputIDComponent=-1, InputIDComponent=-1;
    Point PointInceput, PointSfarsit;
    ...
    void draw(sf::RenderWindow &win)
    {
        if(isDeleted==false)
        {
            sf::Vertex line1[] =
            {
                sf::Vertex(sf::Vector2f(PointInceput.x, PointInceput.y)),
                sf::Vertex(sf::Vector2f(PointInceput.x+(PointSfarsit.x-PointInceput.x)/2, PointInceput.y))
            };
            sf::Vertex line2[] =
            {
                sf::Vertex(sf::Vector2f(PointInceput.x+(PointSfarsit.x-PointInceput.x)/2, PointInceput.y)),
                sf::Vertex(sf::Vector2f(PointInceput.x+(PointSfarsit.x-PointInceput.x)/2, PointSfarsit.y))
            };
            sf::Vertex line3[] =
            {
                sf::Vertex(sf::Vector2f(PointInceput.x+(PointSfarsit.x-PointInceput.x)/2, PointSfarsit.y)),
                sf::Vertex(sf::Vector2f(PointSfarsit.x, PointSfarsit.y))
            };
    ...
    void open(FILE *file)
    {
        fread(&OutputIDComponent, sizeof(int),1,file);
        fread(&InputIDComponent, sizeof(int),1,file);
        fread(&componentOutputPointIndex, sizeof(int),1,file);
        fread(&componentInputPointIndex, sizeof(int),1,file);
        PointInceput.open(file);
        PointInceput.open(file);
    ...
    void save(FILE *file)
    {
        fwrite(&OutputIDComponent, sizeof(int),1,file);
        fwrite(&InputIDComponent, sizeof(int),1,file);
        fwrite(&componentOutputPointIndex, sizeof(int),1,file);
        fwrite(&componentInputPointIndex, sizeof(int),1,file);
        PointInceput.save(file);
        PointInceput.save(file);
...
{
    string type;
    Point previousComponentColtStangaSus;
    float previousScaledAt;
    int lastGradeRotire;


    Point usedComponentNewColtStangaSus;
    float scaledAt=1;
    int rotatedAt=0;
